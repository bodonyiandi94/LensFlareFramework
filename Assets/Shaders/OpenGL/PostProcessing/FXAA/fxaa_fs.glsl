...
void main()
{
    ...
    vec3 pixelBL = textureDR(sScene, uv + (vec2(-1, -1) * texelSize), gl_Layer).rgb;
    vec3 pixelBR = textureDR(sScene, uv + (vec2( 1, -1) * texelSize), gl_Layer).rgb;
    vec3 pixelTL = textureDR(sScene, uv + (vec2(-1,  1) * texelSize), gl_Layer).rgb;
    vec3 pixelTR = textureDR(sScene, uv + (vec2( 1,  1) * texelSize), gl_Layer).rgb;
    vec3 pixelM = textureDR(sScene, uv                              , gl_Layer).rgb;
	...
	vec3 result1 = 0.5 * (textureDR(sScene, (uv + (dir * vec2(1.0 / 3.0 - 0.5))), gl_Layer).xyz +
                          textureDR(sScene, (uv + (dir * vec2(2.0 / 3.0 - 0.5))), gl_Layer).xyz);
                      
	vec3 result2 = result1 * 0.5 + 0.25 * (textureDR(sScene, (uv + (dir * vec2(-0.5))), gl_Layer).xyz +
                                           textureDR(sScene, (uv + (dir * vec2( 0.5))), gl_Layer).xyz);
