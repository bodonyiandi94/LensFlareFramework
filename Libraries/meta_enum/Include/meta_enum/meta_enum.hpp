...
template <typename EnumTypeIn, typename UnderlyingTypeIn, size_t size>
struct MetaEnum
{
	using EnumType = EnumTypeIn;
    using UnderlyingType = UnderlyingTypeIn;
    std::string_view string;
	std::string_view name;
    std::array<MetaEnumMember<EnumTypeIn>, size> members = {};
};
...
constexpr std::string_view parseEnumMemberName(std::string_view memberString)
{
	...
	while (!isAllowedIdentifierChar(*(memberString.data() + nameStart)))
    {
        ++nameStart;
    }
	...
	while (isAllowedIdentifierChar(*(memberString.data() + nameStart + nameSize)))
    {
        ++nameSize;
    }
...
#define meta_enum(Type, UnderlyingType, ...)\
    ...
    constexpr static auto Type##_meta = meta_enum_internal::parseMetaEnum<Type, UnderlyingType, Type##_internal_size()>(#Type, #__VA_ARGS__, []() {\
        using IntWrapperType = meta_enum_internal::IntWrapper<UnderlyingType>;\
        IntWrapperType __VA_ARGS__;\
