...
namespace ImGui
{
	...
	////////////////////////////////////////////////////////////////////////////////
	void Columns(int columns_count, const char* id, ImGuiColumnsFlags flags)
	{
		ImGuiWindow* window = GetCurrentWindow();
		...
		if (conf.values.count > 0) {
			...
			if (conf.grid_x.show) {
				...
				case PlotConfig::Scale::Linear: {
					...
					if (ticks != -1) {
						tick_size = (x_max - x_min) / ((ImMax(1, ticks - 1) * ImMax(1, subticks - 1)));
					}
			...
			for (int i = 0; i < ys_count; ++i) {
				...
				for (int n = 0; n <= res_w; n++) {
					float t0 = n * t_step;
					const size_t v0_idx = (int)(t0 * item_count + 0.5f);
					IM_ASSERT(v0_idx >= 0 && v0_idx < conf.values.count);
					const int v0_idx_offset = conf.values.offset + (v0_idx + 1) % conf.values.count;
					float v0 = ys_list ? ys_list[i][v0_idx] : ysg_list[i](user_data_list[ImMin(i, user_data_count - 1)], 0, v0_idx_offset);

					// Point in the normalized space of our target rectangle
					ImVec2 tp0 = ImVec2(
						rescale(t0, x_min, x_max, conf.scale.type),
						1.0f - ImSaturate((v0 - scale_min) * inv_scale));
					ImVec2 pos = ImLerp(inner_bb.Min, inner_bb.Max, tp0);

					window->DrawList->PathLineTo(pos);
				}
				window->DrawList->PathStroke(col_base, false, conf.line_thickness);
				*/
			}
	...
	////////////////////////////////////////////////////////////////////////////////
	bool sortBoolean(std::string const& a, std::string const& b, TableConfig::Sort::Direction direction)
	{
		return direction == TableConfig::Sort::Ascending ? a < b : b < a;
	}

	////////////////////////////////////////////////////////////////////////////////
	bool sortLexicographical(std::string const& a, std::string const& b, TableConfig::Sort::Direction direction)
	{
		return direction == TableConfig::Sort::Ascending ? a < b : b < a;
	}

	////////////////////////////////////////////////////////////////////////////////
	bool sortIntegral(std::string const& as, std::string const& bs, TableConfig::Sort::Direction direction)
	{
	...
	////////////////////////////////////////////////////////////////////////////////
	bool sortFloatingPoint(std::string const& as, std::string const& bs, TableConfig::Sort::Direction direction)
	{
	...
	using table_sort_callback = bool(*)(std::string const& as, std::string const& bs, TableConfig::Sort::Direction direction);
	static table_sort_callback table_sort_callbacks[] =
	{
		sortBoolean,
		sortIntegral,
		sortIntegral,
	...
	void Table(const char* label, TableConfig conf)
	{
		...
		bool* p_sort = conf.sort.p_sort;
		uint32_t* p_sort_col_id = conf.sort.p_col_id;
		TableConfig::Sort::Direction* p_sort_direction = conf.sort.p_direction;
		if (p_sort == nullptr)
		{
			p_sort = &conf.sort.sort;
		}
		if (p_sort_col_id == nullptr)
		{
			p_sort_col_id = &conf.sort.col_id;
		}
		if (p_sort_direction == nullptr)
		{
			p_sort_direction = &conf.sort.direction;
		}

		bool sort = *p_sort;
		uint32_t sort_col_id = *p_sort_col_id;
		TableConfig::Sort::Direction sort_direction = *p_sort_direction;

		// Sort, if needed
		std::vector<size_t> sortIndices = std::iota<size_t>(conf.rows.values.size(), 0);

		if (conf.rows.values.size() > 0 && *p_sort)
		{
			// Sort the uniform list
			std::sort(sortIndices.begin(), sortIndices.end(), [&](auto const& r1, auto const& r2)
			{
				return table_sort_callbacks[conf.cols.data_types[sort_col_id]](
					conf.rows.values[r1][sort_col_id],
					conf.rows.values[r2][sort_col_id],
					sort_direction);
			});
		}
		...
		{
			colWidths[i] = CalcTextSize(conf.cols.headers[i].c_str()).x;
		}

		// Account for the sort arrow
		if (sort)
		{
			colWidths[sort_col_id] += 0.7f * g.FontSize + g.Style.ItemSpacing.x;
		}
		...
		// Extend the last column to span the remainder of the window width
		colWidths.back() += GetWindowWidth() - totalWidth;
		...
		float rowHeight = CalcTextSize(conf.cols.headers[0].c_str()).y + style.ItemSpacing.y; // height of a single row
		...
		// Window flags
		ImGuiWindowFlags tableChildFlags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse;

		if (BeginChild("TableContents", ImVec2(0.0f, tableHeight), false, tableChildFlags))
		{
			// Generate the column layout
			Columns(conf.cols.count, 0, ImGuiColumnsFlags_NoBorder | ImGuiColumnsFlags_NoResize);
			for (size_t i = 0; i < conf.cols.count; ++i)
			{
				SetColumnWidth(i, colWidths[i] + 2.0f * style.ItemSpacing.x);
			}
			...
			for (size_t i = 0; i < conf.cols.count; ++i)
			{
				...
				if (sort && i == sort_col_id)
				{
					PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(0.0f, g.Style.ItemSpacing.y));
					Arrow(0.7f, sort_direction == TableConfig::Sort::Ascending ? ImGuiDir_Up : ImGuiDir_Down);
					PopStyleVar();
					SameLine();
				}

				Text(conf.cols.headers[i].c_str());

				// Tooltip
				if (IsItemHovered())
				{
					if (i < conf.cols.tooltips.size())
					{
						SetTooltip(conf.cols.tooltips[i].c_str());
					}
					else
					{
						SetTooltip(conf.cols.headers[i].c_str());
					}
				}
				...
				if (selected)
				{
					if (sort && i == sort_col_id)
					{
						if (sort_direction == TableConfig::Sort::Ascending)
						{
							*p_sort_direction = TableConfig::Sort::Descending;
						}
						else if (sort_direction == TableConfig::Sort::Descending)
						{
							*p_sort = false;
						}
					}
					else
					{
						*p_sort = true;
						*p_sort_col_id = i;
						*p_sort_direction = TableConfig::Sort::Ascending;
					}
				}
				NextColumn();
				ImGui::PopID();
			...
			// Add a separator and revert back to a single column mode for the nested child window
			Separator();
			Columns(1);

			// Generate each row
			if (BeginChild("TableRows", ImVec2(0.0f, contentsHeight), false))
			{
				// Generate the column layout
				Columns(conf.cols.count, 0, ImGuiColumnsFlags_NoBorder | ImGuiColumnsFlags_NoResize);
				for (size_t i = 0; i < conf.cols.count; ++i)
				...
				for (size_t rowId = 0; rowId < conf.rows.values.size(); ++rowId)
				{
					...
					for (size_t colId = 0; colId < conf.cols.count; ++colId)
					{
						...
						TableConfig::Column::Alignment alignment = conf.cols.alignments.size() > colId ? conf.cols.alignments[colId] : TableConfig::Column::Left;
	...
	bool RegexFilter(const char* label, FilterRegex& filter, ImTextureID validTexture, ImTextureID invalidTexture)
	{
		...
		// Ignore case button
		ImGui::SameLine();
		filterChanged = ImGui::Checkbox("Ignore Case", &filter.m_ignoreCase) || filterChanged;
		...
		{
			ImGui::SameLine();
			ImGui::ImageSquare(texture, ImVec2(0, 1), ImVec2(1, 0), ImColor(255, 255, 255, 255), ImColor(0, 0, 0, 0));
		}
	...
	static std::unordered_map<std::string, StyleFn> s_styles =
	{
		...
		{ "Unreal",                     [](ImGuiStyle* dst, float dpiScale) { StyleColorsUnreal(dst, dpiScale); } },
