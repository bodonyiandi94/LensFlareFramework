...
namespace Debug
{
	...
	struct FormattedMessage
	{
		...
		FormattedMessage(std::string const& format, const Ts&...  parameters)
		{
			fmt::format_to(m_formatBuffer, format, parameters...);
		}
	...
	struct DebugRegion
	{
		DebugRegion(std::string const& regionName, int currentThread = Threading::numThreads());
		DebugRegion(std::vector<std::string> const& regionNames, int currentThread = Threading::numThreads());
		~DebugRegion();
