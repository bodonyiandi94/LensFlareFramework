...
namespace Debug
{
	...
	void leaveLogRegion(size_t regionNameLength, size_t threadId)
	{
		...
		{
			s_region[threadId] = 
				(regionNameLength > s_region[threadId].size()) ? 
				s_region[threadId].substr(0, s_region[threadId].size() - regionNameLength) :
				std::string();
		});
	}
	...
	////////////////////////////////////////////////////////////////////////////////
	void putLinePrefixConsole(Device& device, LogOutput& log)
	{
		device.m_stream.get() 
			<< "\033[" << log.m_colorCode << "m" 
			<< "[" << DateTime::getDateStringUtf8(DateTime::timeFormatDisplay()) << "]" 
			<< "[" << Threading::currentThreadId() << "]" 
			<< "[" << log.m_nameUpper << "]"
			<< s_region[Threading::currentThreadId()] 
			<< ": ";
	}

	////////////////////////////////////////////////////////////////////////////////
	void putLinePrefixFile(Device& device, LogOutput& log)
	{
		device.m_stream.get() 
			<< "[" << DateTime::getDateStringUtf8(DateTime::timeFormatDisplay()) << "]"
			<< "[" << Threading::currentThreadId() << "]"
	...
	////////////////////////////////////////////////////////////////////////////////
	void putLinePrefixInMemory(Device& device, LogOutput& log)
	{
	...
	////////////////////////////////////////////////////////////////////////////////
	void putLinePrefix(Device& device, LogOutput& log)
	{
		switch (device.m_deviceType)
		{
		case Device::Console:  putLinePrefixConsole(device, log); break;
		case Device::File:     putLinePrefixFile(device, log); break;
		case Device::InMemory: putLinePrefixInMemory(device, log); break;
		}
	}

	////////////////////////////////////////////////////////////////////////////////
	/**  Writes out a log line prefix */
	LogOutput& putLinePrefix(LogOutput& log)
	{
	...
	////////////////////////////////////////////////////////////////////////////////
	/** Logger manipulators. */
	LogOutput& endncr(LogOutput& log)
	{
	...
	///////////////////////////////////////////////////////
	/** Manipulator apply operator */
	LogOutput& operator<<(LogOutput& log, FormattedMessage formattedMessage)
	{
		return log << std::string_view(formattedMessage.m_formatBuffer.data(), formattedMessage.m_formatBuffer.size());
	}
	...
	void printSystemInfo()
	{
		...
		Debug::log_info() << std::string(80, '=') << Debug::end;
		Debug::log_info() << "Config" << Debug::end;
		Config::printValues();

		// - System
		Debug::log_info() << "System" << Debug::end;
		Debug::log_info() << "  - Hardware concurrency: " << std::thread::hardware_concurrency() << Debug::end;
		Debug::log_info() << "  - Number of threads: " << Threading::numThreads() << Debug::end;

		// - OpenGL
		Debug::log_info() << "OpenGL" << Debug::end;
		Debug::log_info() << "  - Version: " << glGetString(GL_VERSION) << Debug::end;
		Debug::log_info() << "  - GLSL Version: " << glGetString(GL_SHADING_LANGUAGE_VERSION) << Debug::end;
		Debug::log_info() << "  - Vendor: " << glGetString(GL_VENDOR) << Debug::end;
		Debug::log_info() << "  - Renderer: " << glGetString(GL_RENDERER) << Debug::end;

		// - Window context
		Debug::log_info() << "Context" << Debug::end;
		Debug::log_info() << "  - Resolution: " << displayParams->width << "x" << displayParams->height << " (" << displayParams->refreshRate << " hz)" << Debug::end;
		Debug::log_info() << "  - Format: " << displayParams->redBits << displayParams->greenBits << displayParams->blueBits << Debug::end;

		// - GPU capabilities
		Debug::log_info() << "GPU Capabilities" << Debug::end;
		for (auto const& capabilityCategoryIt : GPU::s_capabilityAttribs)
		{
			auto const& [category, capabilities] = capabilityCategoryIt;

			Debug::log_info() << "    " << category << Debug::end;
			
			for (auto const& capabilityIt : capabilities)
			{
				auto const& [capabilityId, capability] = capabilityIt;

				Debug::log_info() << "      - " << GPU::enumToString(capability.m_enum) << ": " << capability.m_values << Debug::end;
			}
		}

		// - Tensorflow
		#ifdef HAS_TensorFlow
		Debug::log_info() << "TensorFlow" << Debug::end;
		Debug::log_info() << "  - Version: " << TF_Version() << Debug::end;
		#endif

		// - Eigen
		Debug::log_info() << "Eigen" << Debug::end;
		Debug::log_info() << "  - Number of threads: " << Eigen::nbThreads() << Debug::end;
		Debug::log_info() << std::string(80, '=') << Debug::end;
	}
}