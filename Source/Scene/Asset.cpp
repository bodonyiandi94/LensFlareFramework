...
namespace Asset
{
	...
	std::optional<std::ordered_pairs_in_blocks> loadPairsInBlocks(Scene::Scene& scene, const std::string& fileName)
	{
		...
		Debug::log_debug() << "Full contents:" << Debug::end;
		for (auto const& category : result)
		{
			Debug::log_debug() << category.first << ": " << category.second << Debug::end;
		}
	...
	////////////////////////////////////////////////////////////////////////////////
	bool load3DTexture(Scene::Scene& scene, const std::string& textureName, const std::string& filePath, int numLayers, int columns, int rows)
	{
		Profiler::ScopedCpuPerfCounter perfCounter(scene, filePath);
		...
		texture.m_depth = numLayers;
		texture.m_dimensions = glm::ivec3(layerWidth, layerHeight, numLayers);
		texture.m_format = GL_RGBA8;
		texture.m_layout = GL_RGBA;
		texture.m_minFilter = GL_LINEAR;
		texture.m_magFilter = GL_LINEAR;
		texture.m_wrapMode = GL_CLAMP;
		texture.m_anisotropy = 16.0f;
	...
	////////////////////////////////////////////////////////////////////////////////
	std::string generateFullShaderSource(Scene::Scene& scene, std::vector<std::string> sources, std::string const& mainFileName)
	{
		...
		// Helpers for the start and end strings for a shader source
		auto startSourceLine = [](std::string const& sourceName)
		{
			return "//! Start of contents for source \"" + sourceName + "\"";
		};
		auto endSourceLine = [](std::string const& sourceName)
		{
			return "//! End of contents for source \"" + sourceName + "\"";
		};

		// Write over the common definitions
		shaderSourceStream << startSourceLine("Builtin and User Defines") << std::endl;
		for (size_t i = 0; i < sources.size(); ++i)
		{
			shaderSourceStream << sources[i];
		}
		shaderSourceStream << endSourceLine("Builtin and User Defines") << std::endl;
		...
		auto appendFile = [&](std::string const& fileName, std::vector<std::string>::iterator at)
		{
			...
			std::filesystem::path fullFilePath = EnginePaths::assetsFolder() / fileName;

			// File name, relative to the engine root folder
			std::string fileNameRelative = std::filesystem::relative(fullFilePath, EnginePaths::openGlShadersFolder()).string();
			...
			inserter = startSourceLine(fileNameRelative);
			++inserter;
			
			// Append the source lines
			std::istringstream iss(source.value());
			for (std::string line; std::getline(iss, line); )
			{
				inserter = line;
				++inserter;
			...
			// End content marker
			inserter = endSourceLine(fileNameRelative);
			++inserter;
		};

		// Append the main file
		appendFile(mainFileName, lines.begin());
		...
		for (size_t i = 0; i < lines.size(); ++i)
		{
			...
			else if (std::regex_match(line, capture, s_includeRegex))
			{
				...
				// Only append new includes (no double includes)
				if (processed.find(includeName) == processed.end() || processed[includeName] == false)
				{
					// Append the include
	...
	std::string mainProgramName(std::string const& fileName)
	{
		...
		// Strip the shader the type identifier
		return base.substr(0, base.find_last_of('_'));
	}
		...
		ShaderParameters const& shaderParameters)
	{
		...
		if (source.has_value())
		{
			// Construct definitions for the defines
			std::string builtinDefinitions = generateShaderDefines(generateBuiltinDefines(scene, shaderType, shaderName, fileName));
			
			// Construct definitions for the defines
			std::string defineDefinitions = generateShaderDefines(shaderParameters.m_defines);
			std::string enumDefinitions = generateShaderDefines(shaderParameters.m_enums);
			std::string structDefinitions = generateShaderDefines(shaderParameters.m_structs);

			// Assemble the full shader source
			fullShaderSource = generateFullShaderSource(scene, { builtinDefinitions, defineDefinitions, enumDefinitions, structDefinitions }, fileName );
		}
		else
		{
		...
		Debug::log_trace() << "Shader source successfully generated: " << fileName << Debug::end;

		return fullShaderSource;
	}
		...
		const std::string& customShaderName, ShaderParameters const& shaderParameters)
	{
		...
		{
			if (shaderParameters.m_allowCache)
			{
				return true;
			}
			else
			{
				glDeleteProgram(scene.m_shaders[shaderName].m_program);
			}
		}
