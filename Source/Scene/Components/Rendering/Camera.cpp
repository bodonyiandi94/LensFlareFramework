...
namespace Camera
{
	...
	void handleInput(Scene::Scene& scene, Scene::Object* simulationSettings, Scene::Object* input, Scene::Object* object)
	{
		...
		bool locked = object->component<CameraComponent>().m_locked || 
			input->component<InputSettings::InputComponent>().m_input == 0 || 
			object->component<CameraComponent>().m_settingsLocked > 0 ||
			renderSettings->component<RenderSettings::RenderSettingsComponent>().m_rendering.m_mainCamera != object;
	...
	{
		//return glm::vec2(2.0f * glm::atan(getAspectRatio(resolution) * glm::tan(fovy / 2.0f)), fovy);
		return glm::vec2(getAspectRatio(resolution) * fovy, fovy);
	}

	////////////////////////////////////////////////////////////////////////////////
	glm::vec2 getFieldOfView(Scene::Object* renderSettings, Scene::Object* camera)
	{
		//return glm::vec2(2.0f * glm::atan(getAspectRatio(renderSettings, camera) * glm::tan(camera->m_cameraComponent.m_fovy / 2.0f)), camera->m_cameraComponent.m_fovy);
		return glm::vec2(getAspectRatio(renderSettings, camera) * camera->component<Camera::CameraComponent>().m_fovy, camera->component<Camera::CameraComponent>().m_fovy);
	}
