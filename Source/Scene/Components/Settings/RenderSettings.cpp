...
namespace RenderSettings
{
	...
	void initShaders(Scene::Scene& scene, Scene::Object* object)
	{
		...
		Asset::loadShader(scene, "Misc", "blit_texture");
			...
			// MSAA resolve
		Asset::loadShader(scene, "Misc", "msaa_resolve");
		...
		//Asset::loadShader(scene, "Misc", "oit_resolve");
			...
			// Gaussian blur
		for (int i = 1; i <= 31; ++i)
			loadGaussianBlurShader(scene, i);
	}
	...
	////////////////////////////////////////////////////////////////////////////////
	void updateMainCamera(Scene::Scene& scene, Scene::Object* object)
	{
		// Set the main camera to the default camera if the current one is empty
		if (!isCurrentCameraValid(scene, object))
		{
			const std::string mainCameraName = Config::AttribValue("camera").get<std::string>();
			object->component<RenderSettings::RenderSettingsComponent>().m_rendering.m_mainCamera = Scene::findObject(scene, mainCameraName);
		}
		...
		if (!isCurrentCameraValid(scene, object))
		{
			...
			else
			{
				...
				object->component<RenderSettings::RenderSettingsComponent>().m_rendering.m_mainCamera = newCamera;
			}
		}
	}
	...
	void initObject(Scene::Scene& scene, Scene::Object& object)
	{
		...
		object.component<RenderSettings::RenderSettingsComponent>().m_resolution = GPU::resolution();

		// Extract all the possible resolutions
		object.component<RenderSettings::RenderSettingsComponent>().m_resolutions = 
			std::vector<glm::ivec2>(std::find(RenderSettings::s_allResolutions.begin(), RenderSettings::s_allResolutions.end(), Config::AttribValue("max_resolution").get<glm::ivec2>()), RenderSettings::s_allResolutions.end());

		// Compute the ID of the current resolution
		object.component<RenderSettings::RenderSettingsComponent>().m_rendering.m_resolutionId = std::distance(object.component<RenderSettings::RenderSettingsComponent>().m_resolutions.begin(),
			std::find(object.component<RenderSettings::RenderSettingsComponent>().m_resolutions.begin(), object.component<RenderSettings::RenderSettingsComponent>().m_resolutions.end(), object.component<RenderSettings::RenderSettingsComponent>().m_resolution));

		// Generate and store the display names of the available resolutions
		object.component<RenderSettings::RenderSettingsComponent>().m_resolutionNames.resize(object.component<RenderSettings::RenderSettingsComponent>().m_resolutions.size());
		std::transform(object.component<RenderSettings::RenderSettingsComponent>().m_resolutions.begin(),
			object.component<RenderSettings::RenderSettingsComponent>().m_resolutions.end(),
			object.component<RenderSettings::RenderSettingsComponent>().m_resolutionNames.begin(),
	...
	void generateGui(Scene::Scene& scene, Scene::Object* guiSettings, Scene::Object* object)
	{
		...
		if (ImGui::BeginTabItem("Buffers", activeTab.c_str()))
		{
			...
			ImGui::SliderInt("Voxel Grid Size", &object->component<RenderSettings::RenderSettingsComponent>().m_buffers.m_numVoxels, 8, 512); voxelsChanged |= ImGui::IsItemDeactivatedAfterChange();
			...
			ImGui::SliderInt("MSAA Samples", &object->component<RenderSettings::RenderSettingsComponent>().m_buffers.m_msaa, 1, 16); msaaChanged |= ImGui::IsItemDeactivatedAfterChange();
	...
	void depthPrePassBeginOpenGL(Scene::Scene & scene, Scene::Object * simulationSettings, Scene::Object * renderSettings, Scene::Object * camera, std::string const& functionName, Scene::Object * object)
	{
		...
		glBindTexture(GL_TEXTURE_2D_ARRAY, scene.m_gbuffer[renderSettings->component<RenderSettings::RenderSettingsComponent>().m_gbufferRead].m_depthTexture);
	...
	void lightingBeginOpenGL(Scene::Scene& scene, Scene::Object* simulationSettings, Scene::Object* renderSettings, Scene::Object* camera, std::string const& functionName, Scene::Object* object)
	{
		...
		{
			glActiveTexture(GPU::TextureEnums::TEXTURE_DEPTH_ENUM);
			glBindTexture(GL_TEXTURE_2D_MULTISAMPLE_ARRAY, scene.m_gbuffer[renderSettings->component<RenderSettings::RenderSettingsComponent>().m_gbufferWrite].m_depthTextureMsaa);
			glActiveTexture(GPU::TextureEnums::TEXTURE_ALBEDO_MAP_ENUM);
			glBindTexture(GL_TEXTURE_2D_MULTISAMPLE_ARRAY, scene.m_gbuffer[renderSettings->component<RenderSettings::RenderSettingsComponent>().m_gbufferWrite].m_colorTextureMsaa);
			glActiveTexture(GPU::TextureEnums::TEXTURE_NORMAL_MAP_ENUM);
			glBindTexture(GL_TEXTURE_2D_MULTISAMPLE_ARRAY, scene.m_gbuffer[renderSettings->component<RenderSettings::RenderSettingsComponent>().m_gbufferWrite].m_normalTextureMsaa);
			glActiveTexture(GPU::TextureEnums::TEXTURE_SPECULAR_MAP_ENUM);
			glBindTexture(GL_TEXTURE_2D_MULTISAMPLE_ARRAY, scene.m_gbuffer[renderSettings->component<RenderSettings::RenderSettingsComponent>().m_gbufferWrite].m_specularTextureMsaa);
		}
		else
		{
			glActiveTexture(GPU::TextureEnums::TEXTURE_DEPTH_ENUM);
			glBindTexture(GL_TEXTURE_2D_ARRAY, scene.m_gbuffer[renderSettings->component<RenderSettings::RenderSettingsComponent>().m_gbufferWrite].m_depthTexture);
			glActiveTexture(GPU::TextureEnums::TEXTURE_ALBEDO_MAP_ENUM);
			...
			glActiveTexture(GPU::TextureEnums::TEXTURE_NORMAL_MAP_ENUM);
			glBindTexture(GL_TEXTURE_2D_ARRAY, scene.m_gbuffer[renderSettings->component<RenderSettings::RenderSettingsComponent>().m_gbufferWrite].m_normalTexture);
			glActiveTexture(GPU::TextureEnums::TEXTURE_SPECULAR_MAP_ENUM);
			glBindTexture(GL_TEXTURE_2D_ARRAY, scene.m_gbuffer[renderSettings->component<RenderSettings::RenderSettingsComponent>().m_gbufferWrite].m_specularTexture);
		}
	}
