...
namespace ToneMap
{
	...
	{
		// Reinhard operator
		if (object->component<ToneMap::TonemapComponent>().m_operator == ToneMap::TonemapComponent::Clamp)
		...
		// Filmic operator
		if (object->component<ToneMap::TonemapComponent>().m_operator == ToneMap::TonemapComponent::Filmic)
	...
	namespace ToneMapCurveChart
	{
		...
		float chartDataGenerator(const void* pPayload, int index, float x)
		{
			...
			return evalCurve(*payload.m_scene, payload.m_object, computeSamplePosition(payload, index));
		}
		...
		void chartTooltipGenerator(const void* pPayload, int v_idx, float x, float y)
		{
			...
			float samplePos = computeSamplePosition(payload, v_idx);

			// Sample the curve
			float sampleVal = evalCurve(*payload.m_scene, payload.m_object, samplePos);
		...
		void generateXTickLabel(const void* pPayload, char* buffer, int buf_size, float val)
		{
			...
			// Position of the sample along the track
			float samplePos = computeSamplePosition(payload, int(val));

			// Construct the label
			sprintf(buffer, "%.2f", samplePos);
		}
		...
		void generateChart(Scene::Scene& scene, Scene::Object* guiSettings, Scene::Object* object)
		{
			...
			overlayStream << ToneMap::TonemapComponent::ToneMapOperator_value_to_string(object->component<ToneMap::TonemapComponent>().m_operator);
			...
			plotConfig.scale.min = 0.0f;
			plotConfig.scale.max = 1.0f;
			plotConfig.v_lines.show = false;
	...
	void generateGui(Scene::Scene& scene, Scene::Object* guiSettings, Scene::Object* object)
	{
		...
		if (ImGui::BeginTabItem("Color Table", activeTab.c_str()))
		{
			...
			{
				Asset::load3DTexture(scene, object->component<ToneMap::TonemapComponent>().m_colorTable, object->component<ToneMap::TonemapComponent>().m_colorTable, 16);
