...
namespace Scene
{
	...
	Object& createObject(Scene& scene, const std::string& baseName, unsigned long long components, ObjectInitializer initializer)
	{
		...
		{
			Debug::log_debug() << 
				"Adding " << componentNames()[componentType] << " (with ID #" << componentType << ", mask " << std::bit_mask(componentType) << ") " <<
				"to object of type " << objectNames()[components] << " (with mask " << object.m_componentList << ")" <<
				Debug::end;
	...
	{
		auto const& objects = filterObjects(scene, objectType, true, false);
		return objects.empty() ? nullptr : objects[0];
	}
	...
	{
		return scene.m_firstObjects.empty() ? findFirstObjectSlow(scene, mask) : scene.m_firstObjects[mask];
	}
	...
	{
		Debug::log_trace() << "Generating texture: " << textureName << Debug::end;
		...
		GLenum minFilter, GLenum magFilter, GLenum wrapMode, std::string const& shaderName, GLenum bindingId)
	{
		...
		return generateTextureImpl(scene, textureName, textureType, width, height, depth, format, layout, minFilter, magFilter, wrapMode, bindingId, "", GL_UNSIGNED_BYTE, nullptr, shaderName);
	}
	...
	{
		Debug::log_trace() << "Creating GBuffer [" << bufferWidth << "X" << bufferHeight << "], " << numLayers << " layers, " << samples << " samples." << Debug::end;
		...
		Debug::log_trace() << "Successfully created GBuffer." << Debug::end;
	...
	{
		Debug::log_trace() << "Creating Voxel grid [" << bufferWidth << "X" << bufferHeight << "X" << bufferDepth << "]" << Debug::end;
	...
	////////////////////////////////////////////////////////////////////////////////
	bool createGPUBuffer(Scene& scene, const std::string& bufferName, GLenum bufferType, bool immutable, bool indexed, GLuint bindingId, GLenum flags, GLsizeiptr storageSize, GLsizeiptr elementSize)
	{
	...
	bool resizeGPUBuffer(Scene& scene, const std::string& bufferName, size_t newSize, bool growOnly)
	{
		...
		{
			Debug::log_trace() << "Skipping GPU buffer resize for buffer \"" << bufferName << "\";"
		...
		Debug::log_trace() << "Resizing GPU buffer \"" << bufferName << "\" of size " << Units::bytesToString(newSize) << "(s)." << Debug::end;
		...
		Debug::log_trace() << "GPU buffer \"" << bufferName << "\" successfully resized." << Debug::end;
	...
	void buildGraph(std::string const& name, R registrators, G& graph)
	{
		...
		while (registrators.size() > 0)
		{
			...
			if (anyProcessed == false)
			{
				...
				Debug::log_trace() << "Entries not placed: " << Debug::end;
				for (auto const& registratorIt : registrators)
				{
					auto [objectId, registrator] = registratorIt;
					auto [refId, relation, priority, fn] = registrator;
					Debug::log_trace() << " - " << nodeName(registrator) << " (" << priority << ", " << referenceName(registrator) << ")" << Debug::end;
				}
				break;
			}
		}
	...
	void rebuildFirstObjectAccelStructure(Scene& scene)
	{
		...
		// Override it with the new values
		for (auto objectType : objectTypes())
		{
			scene.m_firstObjects[objectType] = findFirstObjectSlow(scene, objectType);
		}
	}
	...
	void renderScene(Scene& scene, ObjectRenderFunctions const& renderFunctions, ObjectRenderParameters renderParameters)
	{
		...
		// Clear the disabled function list
	...
	void renderScene(Scene& scene)
	{
		...
		renderParameters.m_camera = renderParameters.m_renderSettings->component<RenderSettings::RenderSettingsComponent>().m_rendering.m_mainCamera;
		...
		Debug::log_trace() << "Rendering scene: " << scene.m_name << Debug::end;
		Debug::log_trace() << "Main camera: " << renderParameters.m_camera->m_name << Debug::end;
	...
	void mainLoop(Scene& scene)
	{
		...
		while (!glfwWindowShouldClose(scene.m_context.m_window))
		{
			...
			{
				Debug::DebugRegion region({ "Update" });
				...
				// Load new resources that haven't been loaded yet
				loadResources(scene);
			}

			// Render the scene
			{
				Debug::DebugRegion region({ "Render" });
