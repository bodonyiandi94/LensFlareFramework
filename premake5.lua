...
local WORKSPACE_NAME = "LensFlareFramework"

-- Name of the project
local PROJECT_NAME = "LensFlareFramework"
        ...
        if _OPTIONS["disable_cmdargs"] == nil then
            ...
            for _, filename in pairs(source_files) do
                for line in io.lines(filename) do
                    ...
                    if (match_begin) then
                        ...
                        var_group, var_name = var_values[1], var_values[2]
                        --print(table.tostring(var_values))
                        --print(filename, var_group, var_name, var_value)
                        if config_vars_[var_group] == nil then
                            config_vars_[var_group] = {}
                        end
                        config_vars_[var_group]["var_name"] = var_name
                        if config_vars_[var_group]["var_values"] == nil then
                            config_vars_[var_group]["var_values"] = {}
                        end
                        for i = 3, #var_values do
                            config_vars_[var_group]["var_values"][#config_vars_[var_group]["var_values"] + 1] = var_values[i]
                        end
            ...
            for group_name, group in pairs(config_vars_) do
                config_vars[#config_vars + 1] = {
                    ...
                    var_values = group["var_values"]
                }
            end
            table.sort(config_vars, function(a,b) return a["var_group"] < b["var_group"] end)
    
            -- build the cmd line argument definitions from the list
            command_line_arguments = {}
            args_md5 = md5.new()
            for _, cvar in ipairs(config_vars) do
                command_line_arguments[#command_line_arguments + 1] = make_folder(
                    cvar["var_group"], make_args(cvar["var_name"], cvar["var_values"]))
            ...
            --print(table.tostring(config_vars, 3))
            ...
            if (cmd_args_current_version ~= cmd_args_file_version) then
                print("Updating command line argument list")
